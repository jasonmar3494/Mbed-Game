...
int main() {
    ...
    while(1) {
        ...
        while(1){
		    ...
		    double t = 0;
		    double vx = power*cos((angle*PI/180));
             	    double vy = power*sin((angle*PI/180));
	            while(t<10){
		    	double xt = floor(vx*t);
                    	double yt = floor(((vy/vx)*xt) - ((0.5)*(GRAVITY)*((xt/vx)*(xt/vx))));
			int ytt = (int) yt;
			int xtt = (int) xt;
			for(int j=0;j<i;j++){
				if((world[j*4+2] == ytt) && (world[j*4+3] == (xtt+1))){
					if(world[j*4+5]==1){
						updateShot(ytt,xtt,0);
						colorTile(world[j*4+2],world[j*4+3],world[j*4+5]-1);
						break;
					}
					else{
						updateShot(ytt,xtt,0);
						deleteTile(world[j*4+2],world[j*4+3]);
						break;
					}
				}
				else if((world[j*4+2] == (ytt-1)) && (world[j*4+3] == xtt)){
					if(world[j*4+5]==1){
						updateShot(ytt,xtt,0);
						colorTile(world[j*4+2],world[j*4+3],world[j*4+5]-1);
						break;
